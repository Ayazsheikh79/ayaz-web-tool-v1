...
export async function POST (req: Request) {
    ...
    try {
        ...
        const limit = await prisma.envatoLimit.findUnique({
            where: {
                userId
            }
        });

        const monthlyLimit = await prisma.envatoMonthlyLimit.findUnique({
            where: {
                userId
            }
        });
        ...
        if (limit.envato <= 0 && monthlyLimit.envato <= 0) {
            return Response.json({
        ...
        if (limit.envato <= 0) {
            await prisma.envatoMonthlyLimit.update({
                where: {
                    userId
                },
                data: {
